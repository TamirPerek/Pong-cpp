...
public:
	...
	explicit Player(const WindowSize& xWindowSize) 
		: UIElement{ SDL_Rect{0, 0, xWindowSize.w / 40, xWindowSize.h / 4} },
...
public:
	...
	void update(const WindowSize& xWindowSize, const unique_renderer_t& xRenderer) noexcept
	{
		...
		if (SDL_RenderCopy(xRenderer.get(), Message.get(), nullptr, static_cast<const SDL_Rect*>(*this)) != 0)
